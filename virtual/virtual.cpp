...
void devfn(F f, std::tuple<Args...> args)
{
#pragma omp target
	{
...
template<typename T>
struct Deriv<T, true> : public Base<T>
{
	using Base<T>::Base;

	virtual void f()
	{
		devfn(Kernel<42>(), std::make_tuple(1));
	}
};

template<typename T>
struct Deriv2 : public Base<T>
{
	using Base<T>::Base;

	virtual void f()
	{
		devfn(Kernel<2>(), std::make_tuple(2));
	}
};

template<typename T>
void callBase(Base<T>* b)
{
	b->f();
}
...
int main()
{
	auto d = std::make_unique<Deriv<int, true>>();
	callBase(d.get());
}
